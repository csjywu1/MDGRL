...
//        targets = IO.readTargetsFromRules(ruleFile, nodeIndex);
    ...
    private void specialization(Context context, Set<Pair> trainPairs, Set<Pair> validPairs) {
        ...
        for (int i = 0; i < tasks.length; i++) {
            tasks[i] = new SpecializationTask(i, graph, abstractRuleQueue, trainPairs, validPairs,
                    trainObjToSub, trainSubToObj, validObjToSub, validSubToObj, context);
        }
        try {
    ...
    static class SpecializationTask extends Thread {
            ...
            try(Transaction tx = graph.beginTx()) {
                while (!abstractRuleQueue.isEmpty() && !GlobalTimer.stopSpec()) {
                    ...
                    if(abstractRule != null) {
                        ...
                        Specialization(abstractRule, graph, trainPairs, validPairs, trianAnchoringToOriginals, validAnchoringToOriginals, context);
                    }
                }
                tx.success();
            }
        ...
        public void Specialization(Rule rule, GraphDatabaseService graph, Set<Pair> groundTruth, Set<Pair> validPair
                , Multimap<Long, Long> anchoringToOriginal, Multimap<Long, Long> validAnchoringToOriginals
                , Context context) {
            CountedSet<Pair> bodyGroundings = GraphOps.bodyGroundingCoreAPI(graph, rule);
            ...
            if(rule.closed) {
                ...
                for (Long anchoring : anchoringToOriginal.keySet()) {
                    ...
                    if(evaluateRule(HAR, anchoringToOriginal.get(anchoring), validAnchoringToOriginals.get(anchoring), groundingOriginals)) {
                        valid = true;
                    ...
                    for (Long original : anchoringToOriginal.get(anchoring)) {
                        ...
                        for (Long tail : originalToTail.get(original)) {
                            ...
                            if(!visited.contains(candidate) && !trivialCheck(rule, anchoring, tail)) {
                                ...
                                if (evaluateRule(BAR, anchoringToOriginal.get(anchoring), validAnchoringToOriginals.get(anchoring), tailToOriginal.get(tail))) {
                                    valid = true;
        ...
        private boolean evaluateRule(Rule rule, Collection<Long> originals, Collection<Long> validAnchoringToOriginals, Collection<Long> groundingOriginals) {
            ...
            for (Long groundingOriginal : groundingOriginals) {
                ...
                if(originals.contains(groundingOriginal))
                    ...
                    validTotalPredictions++;
                    if(validAnchoringToOriginals.contains(groundingOriginal))
                        validPredictions++;
                }
            }
