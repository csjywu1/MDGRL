...
public abstract class Engine {
    ...
    public static int vector_len =100;
    public static double[][] relation_vec;
    public static double[][] entity_vec;
    public static Map<String,Integer> entity2id;
    public static Map<String,Integer> relation2id;
    ...
    public void generalization(Set<Pair> trainPairs, Context context) {
        ...
        RuleProducer producer = new RuleProducer(0, ruleQueue, trainPairs, visitedTrainPairs, graph, consumer);
        try {
            producer.join();
            consumer.join();
    ...
    public void specialization(Context context, Set<Pair> trainPairs, Set<Pair> validPairs, File ruletempFile) throws IOException, InterruptedException {
        ...
        /*
         * abstractRuleQueue，是从context中获取closedRules，openRule
         ...
         */
        RuleWriter tempFileWriter = new RuleWriter(0, ruletempFile, tempFileContents, true);
        RuleWriter ruleFileWriter = new RuleWriter(0, ruleFile, ruleFileContents, true);
        PrintWriter writer = new PrintWriter(new FileWriter(ruletempFile, true));
        PrintWriter writer1 = new PrintWriter(new FileWriter(ruleFile, true));
        SpecializationTask  tasks = new SpecializationTask(0, graph, abstractRuleQueue
                , trainPairs, validPairs, trainObjToSub, trainSubToObj, validObjToSub, validSubToObj
                , context, tempFileContents, ruleFileContents,tempFileWriter,ruleFileWriter,writer,writer1);
        tasks.join();
    ...
    public void ruleApplication(Context context, File ruletempFile) {
        Logger.println("\n# Start Rule Application", 2);
        NumberFormat f = NumberFormat.getNumberInstance(Locale.US);
        long s = System.currentTimeMillis();
        context.initPredictionMap();
        /*
         * 从各类型的关系生成的规则文件中读取规则，存放到abstractRuleQueue当中
         * 包含长度为1的instantiated rule，后接许多附属的HAR或者BAR
         * 包含长度大于1的closedrule和instantiated rule，其中instantiated rule，后接许多附属的HAR或者BAR
         * RuleApplicationTask是监听者，监听存放到abstractRuleQueue中的信息
         */
        BlockingQueue<Rule> abstractRuleQueue = new LinkedBlockingDeque<>(100000);
        /*
         * !!!!!!!!!!!!!
         * 从tempFile中读取规则，并检验是否过拟合
         */
//        GlobalTimer.setApplStartTime(System.currentTimeMillis());

        RuleReader reader = new RuleReader(0, ruletempFile, abstractRuleQueue, context);
        RuleApplicationTask task = new RuleApplicationTask(0, graph, abstractRuleQueue, context, reader);
        try {
            task.join();
            reader.join();
        } catch (InterruptedException e) {
        ...
        /*
         * Applied Rules：用到多少rule生成query
         * Predictions:query的总数，query中包含newTriple
         */
    ...
    static class SpecializationTask extends Thread {
        ...
        Set<Pair> trainPairs;
        Set<Pair> validPairs;
        Multimap<Long, Long> trainObjToSub;
        Multimap<Long, Long> trainSubToObj;
        Multimap<Long, Long> validObjToSub;
        Multimap<Long, Long> validSubToObj;
        BlockingQueue<String> ruleFileContents;
        RuleWriter tempFileWriter;
        RuleWriter ruleFileWriter;
        PrintWriter writer;
        PrintWriter writer1;
        ...
        public SpecializationTask(int id
                ...
                , Context context
                , BlockingQueue<String> tempFileContents
                , BlockingQueue<String> ruleFileContents,RuleWriter tempFileWriter,RuleWriter ruleFileWriter,PrintWriter writer,PrintWriter writer1) {
            ...
            this.trainObjToSub = trainObjToSub;
            this.trainSubToObj = trainSubToObj;
            this.context = context;
            ...
            this.ruleFileWriter=ruleFileWriter;
            this.writer=writer;
        ...
        public void run() {
            try(Transaction tx = graph.beginTx()) {
                //&& !GlobalTimer.stopSpec()
                while (!abstractRuleQueue.isEmpty()&&context.getTotalInsRules()<Settings.INS_RULE_CAP&&!GlobalTimer.stopSpec()) {
                    Template abstractRule = (Template) abstractRuleQueue.poll();
                    if(abstractRule != null) {
                        Multimap<Long, Long> trianAnchoringToOriginals = abstractRule.isFromSubject() ? trainObjToSub : trainSubToObj;
                        Multimap<Long, Long> validAnchoringToOriginals = abstractRule.isFromSubject() ? validObjToSub : validSubToObj;
                        abstractRule.specialization(graph, trainPairs, validPairs
                                , trianAnchoringToOriginals, validAnchoringToOriginals, context
                                , ruleFileContents, tempFileContents,tempFileWriter,ruleFileWriter,writer,writer1);
                    }
                }
                tx.success();
            } catch (InterruptedException e) {
    ...
    static class RuleApplicationTask extends Thread {
        ...
        public void run() {
            try(Transaction tx = graph.beginTx()) {
                while ((ruleReader.isAlive() || !abstractRuleQueue.isEmpty())&& context.predictionMapSize() < Settings.SUGGESTION_CAP) {
                    Template abstractRule = (Template) abstractRuleQueue.poll();
                    if (abstractRule != null) {
                        /*
                         * 统计使用来生成过答案的规则
    ...
    static class RuleReader extends Thread {
        ...
        RuleReader(int id, File ruletempFile, BlockingQueue<Rule> abstractRuleQueue, Context context) {
            this.id = id;
            this.ruletempFile = ruletempFile;
            this.abstractRuleQueue = abstractRuleQueue;
            this.context = context;
            start();
        }
            ...
            try (LineIterator l = FileUtils.lineIterator(ruletempFile)) {
                while(l.hasNext()) {
                    ...
                    if(line.startsWith("ABS: ")) {
                        /*
                         * 获取rule的index
                         * 举例，就是这个28900的数字就是index
                         * ABS: 28900	OAR	CanFollow(X,Y) <- CanFollow(V1,Y,1), ParentOf(V1,V2,0), instanceof(V3,V2,1)
                         */
                        int index = Integer.parseInt(line.split("ABS: ")[1].split("\t")[0]);
                        /*
                         * 从indexRule中获取，获取rule
                         */
                        Template rule = (Template) context.getRule(index);
                        /*
                         * 从头到尾执行不会出现rule=null的情况，出现这种情况，一般是前面context当中没有保存规则，直接拿tempfile直接生成rule变量
                         */
                        if(rule == null) {
                            rule = new Template(line.split("ABS: ")[1]);
                        }
                        /*
                         * 只用Head为指定Target的rule，来生成答案
                         * 在前面生成答案的时候有设置，Settings.TARGET = target;
                         */
                        ...
                        if(!rule.isClosed()) {
                            ...
                            for (String s : insRuleLine.split("\t")) {
                                /*
                                 * 举例，分成两类
                                 * 1. 一类是HAR!!!!!!!!!!在SimpleInsRule前面标为OAR
                                 * OAR	ChildOf(X,Y) <- ChildOf(X,V1), CanFollow(V2,V1), instanceof(V3,V2) 前面是base的内容------	[18]	0.46667 后面是HAR的内容
                                 * 2. 另一类是BAR!!!!!!!!!!在SimpleInsRule前面标准为OAR
                                 * OAR	ChildOf(X,Y) <- ChildOf(X,V1), CanFollow(V2,V1), instanceof(V3,V2) 前面是base的内容------	[18,4099]	0.80769 后面是BAR的内容
                                ...
                                /*
                                 * 检测附属的HAR或者BAR是否满足规则，只有满足才添加到insRules集合当中
                                 * 训练集中的smoothedConf需要大于validPrecision：在验证集当中的standardConf
                                 */
...
//                                }
                            ...
                            /*
                             *假设instantiatedRule没有附属的HAR或者BAR,不用它生成答案
                             */
                                continue;
                        } else {
                            /*
...
//                                    continue;
                        ...
                        /*
                         *将符合条件的closedRule以及instantiatedRule (附带HAR以及BAR) 放进abstractRuleQueue当中
                         */
    ...
    public static class RuleWriter extends Thread {
        File file;
        public BlockingQueue<String> contentQueue;
        boolean append;
        int id;

        public RuleWriter(int id, File file, BlockingQueue<String> contents, boolean append) {
            super("RuleWriter-" + id);
            this.append = append;
            this.id = id;
            this.file = file;
